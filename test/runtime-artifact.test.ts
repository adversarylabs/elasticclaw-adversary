...
test("the bundled runtime executes without node_modules", async () => {
  const artifact = await mkdtemp(join(tmpdir(), "elasticclaw-artifact-"));
  const target = await mkdtemp(join(tmpdir(), "elasticclaw-target-"));
  ...
  assert.equal(result.adversary.name, "factory/elasticclaw");
  assert.equal(result.adversary.version, "0.0.13");
